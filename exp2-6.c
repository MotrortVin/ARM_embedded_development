...
volatile uint16_t systick_1s_couter,systick_200ms_couter; //counter of 1s or 200ms
volatile uint8_t	systick_1s_status,systick_200ms_status; //the state of the counter

uint32_t hour = 23, min = 58, sec =58;
bool press_sw1 = false, press_sw2 = false;

int main(void)
{
	
	volatile uint32_t now, cnt=0; //now means"minute*100+second",cnt is the transform number,can be 1000,100,10,1
	volatile uint8_t num; //num is the No of the seg
  ...
  IntMasterEnable();	//enable the intterupt
	...
	while (1)
	{
		...
		//show the time
		I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,(uint8_t)(0));				//clear
		...
		else  //to show the signal "-"
		{
			I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT1,0x40);								//write port 1: "-"				
			I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,(uint8_t)(0x24));		//write port 2: No.3
			num++;
		}

		//update time
		if (systick_1s_status) //1s counter is on!
		{
			systick_1s_status	= 0; //reset
			if (!press_sw1 && !press_sw2) ++sec;   //when all is not bitten
		}

		if (systick_200ms_status) //200ms counter is on!
		{
			systick_200ms_status	= 0; //reset
			if (press_sw1) ++sec;
			if (press_sw2) ++min;
		}
		...
		if (GPIOPinRead(GPIO_PORTJ_BASE,GPIO_PIN_0)	== 0)	{					//USR_SW1-PJ0 pressed
			...
			if (press_sw1) ++sec; //you give up it at the first time
			press_sw1 = false;
		}
		if (GPIOPinRead(GPIO_PORTJ_BASE,GPIO_PIN_1)	== 0)	{					//USR_SW1-PJ0 pressed
			press_sw2 = true;
		} else { 	//USR_SW1-PJ1 released
			if (press_sw2) ++min; //you give up it at the first time
			press_sw2 = false;
		}
