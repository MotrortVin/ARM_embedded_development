...
#define GPIO_FLASHTIME						300				//300mS,GPIO定时时长
...
volatile uint16_t systick_10ms_couter=SYSTICK_FREQUENCY/100;	//10ms计时器
volatile uint16_t systick_100ms_couter=SYSTICK_FREQUENCY/10; 	//100ms计时器
volatile uint8_t	systick_10ms_status,systick_100ms_status; 	//10ms和100ms计时状态
...
int main(void)
{
  ...
  IntMasterEnable();	//处理器中断使能
	...
	while (1)
	{
		...
		{
			systick_10ms_status		= 0; //重置10ms定是状态
			...
			//利用10ms定时刷新数码管，可以让视觉同同时看到两个码管值
			I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2,0); //清空数码管，防止拖影
			if (cnt < 7 || k == 0) {
			...
			} else {  //To show 1 and 8 at the same time
			...
			//LED跑马灯
				if (cnt < 7)
					...
					I2C0_WriteByte(PCA9557_I2CADDR,PCA9557_OUTPUT,~((3<<cnt)|0x1));  //we use "|" to add the situation 1and8
			
		}
		
		if (systick_100ms_status) //100ms long time have passed
		{
			systick_100ms_status	= 0; //to reset the state of counting 100ms
